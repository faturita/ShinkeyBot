...
- (void)doSend
{
	...
	else
	{
		...
		if (currentSend->addressFamily == AF_INET)
		{
			result = sendto(socket4FD, (const void *)buffer, (size_t)length,(int) 0, (const struct sockaddr *)dst, (socklen_t)dstSize);
			LogVerbose(@"sendto(socket4FD) = %d", result);
		}
		else
		{
			result = sendto(socket6FD, (const void *)buffer, (size_t)length, (int)0, (const struct sockaddr *)dst, (socklen_t)dstSize);
